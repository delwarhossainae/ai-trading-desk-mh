...
function normalizeTakeProfits(value) {
  if (Array.isArray(value)) {
    return { tp1: sanitizeText(value[0]), tp2: sanitizeText(value[1]), tp3: sanitizeText(value[2]) };
  }
  if (value && typeof value === 'object') {
    return {
      tp1: sanitizeText(value.tp1 ?? value.tp_1 ?? value.first ?? value.target1 ?? value.target_1 ?? value[0]),
      tp2: sanitizeText(value.tp2 ?? value.tp_2 ?? value.second ?? value.target2 ?? value.target_2 ?? value[1]),
      tp3: sanitizeText(value.tp3 ?? value.tp_3 ?? value.third ?? value.target3 ?? value.target_3 ?? value[2])
    };
  }
  const text = sanitizeText(value);
  return { tp1: text, tp2: '', tp3: '' };
}
...
function normalizeAnalysisResult(raw = {}, context = {}) {
  ...
  return {
    ...
    takeProfits: {
      ...normalizeTakeProfits(raw.takeProfits)
    },

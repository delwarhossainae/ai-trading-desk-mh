...
async function buildMarketContext(request = {}) {
  ...
  const economicEvents = await getEconomicEvents({ ...request, asset: asset.canonicalAsset, assetClass: asset.assetClass }).catch(() => ({ ok: false, configured: false, events: [], riskSummary: 'Latest news/calendar data could not be verified.', message: 'Economic calendar data could not be loaded.' }));
  const economicRisk = { configured: Boolean(economicEvents.configured), verified: Boolean(economicEvents.ok), summary: economicEvents.riskSummary || economicEvents.message || 'Latest news/calendar data could not be verified.', events: (economicEvents.events || []).slice(0, 10) };
  const riskFilters = buildRiskFilters({ marketConfigured: configured, currentPrice, summaries, selected: selectedTimeframe, economicEvents, warnings });
  return { ok: riskFilters.marketDataAvailable, configured, provider, asset: request.asset, canonicalAsset: asset.canonicalAsset, assetClass: asset.assetClass, symbol: resolveSymbol({ asset: asset.canonicalAsset }), selectedTimeframe, strategyMode: request.strategyMode, riskProfile: request.riskProfile, analysisDepth: request.analysisDepth, asOf, currentPrice, spreadStatus: 'unknown', session: sessionStatus(asset.assetClass), dataCompleteness: { requestedTimeframes: plan, availableTimeframes: Object.entries(summaries).filter(([,s]) => s.candleCount > 0).map(([tf])=>tf), warnings }, timeframes: summaries, economicRisk, macroContext: { summary: economicRisk.summary }, signalSummary: signalSummary(summaries, selectedTimeframe), riskFilters, recentCandles };
}

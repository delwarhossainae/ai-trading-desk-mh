...
function requireJsonContentType(req, res) {
  if (req.method === 'POST' && !hasJsonContentType(req)) {
    res.status(415).json({ ok: false, success: false, message: 'Content-Type must be application/json.' });
    return true;
  }
  return false;
}
...
function applyCors(req, res) {
  ...
  if (isProduction() && !allowedOrigins.length) {
    if (req.method === 'OPTIONS') res.status(403).json({ ok: false, message: SENSITIVE_ORIGIN_MESSAGE });
    else res.status(403).json({ ok: false, message: SENSITIVE_ORIGIN_MESSAGE });
    return true;
  }
  ...
  if (requestOrigin && !originIsAllowed(requestOrigin)) { res.status(403).json({ ok: false, message: 'Invalid request origin.' }); return true; }
  return false;
}
function rejectOversized(req, res) { const length = Number(req.headers['content-length'] || 0); if (length > MAX_BODY_BYTES) { res.status(413).json({ ok: false, message: 'Invalid request body' }); return true; } return false; }
...
async function rateLimit(req, res, routeKey = 'default') {
  ...
  if (limited) { res.status(429).json({ ok: false, success: false, message: RATE_LIMIT_MESSAGE }); return true; }
  return false;
}
...
async function guardRequest(req, res, options = {}) {
  ...
  if (req.method !== 'POST') { res.status(405).json({ ok: false, message: 'Invalid request method.' }); return true; }

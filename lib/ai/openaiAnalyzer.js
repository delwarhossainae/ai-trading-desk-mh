...
function buildAnalysisPrompt(payload) {
  return [
    ...
    'Always allow No Trade, Watch, or Invalid Setup. If evidence is incomplete or unclear, choose No Trade, Watch, or Invalid Setup.',
    ...
    '',
    'Return only valid JSON with keys: decision, bias, confidence, score, entryZone, stopLoss, takeProfits, riskReward, timeframeSummary, marketStructure, economicRisk, reasons, invalidations, riskWarning, scorecard. decision must be one of No Trade, Watch, Invalid Setup, Buy Setup, or Sell Setup.'
  ].join('\n');
}
...
async function analyzeWithOpenAI(payload) {
  if (!process.env.OPENAI_API_KEY) {
    return { ok: false, message: 'AI analysis service is not configured.', ...fallbackNoTrade(payload, 'AI analysis service is not configured.') };
  }
  ...
  if (!response.ok) {
    return { ok: false, message: 'Analysis temporarily unavailable.', ...fallbackNoTrade(payload, 'Analysis temporarily unavailable.') };
  }

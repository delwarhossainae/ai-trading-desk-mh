...
const PROVIDER_CONFIG = {
  openai: { key: 'OPENAI_API_KEY', model: 'OPENAI_MODEL', fallbackModel: 'gpt-5-mini' },
...
function providerStatuses() {
  return Object.fromEntries(Object.entries(PROVIDER_CONFIG).map(([id, config]) => {
    const configured = id === 'microsoft'
      ? Boolean(process.env.MICROSOFT_AI_API_KEY || process.env.AZURE_OPENAI_API_KEY)
...
function buildAnalysisPrompt(payload) {
  ...
  return [
    ...
    `Economic risk/events: ${JSON.stringify(payload.economicRisk || payload.economicEvents || {})}`,
    '',
...
async function analyzeWithOpenAI(payload) {
  ...
  const model = process.env.OPENAI_MODEL || PROVIDER_CONFIG.openai.fallbackModel;
  const response = await fetch('https://api.openai.com/v1/responses', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${process.env.OPENAI_API_KEY}` }, body: JSON.stringify({ model, input: buildAnalysisPrompt(payload), temperature: 0.2, text: { format: { type: 'json_object' } } }) });
  const data = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error('Analysis temporarily unavailable.');
  const text = data.output_text || data.output?.flatMap((item) => item.content || []).map((item) => item.text || '').join('') || '{}';
  return normalizeProviderResult(extractJson(text), 'openai', model);
}
...
async function analyzeWithAnthropic(payload) {
  ...
  const response = await fetch('https://api.anthropic.com/v1/messages', { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-api-key': process.env.ANTHROPIC_API_KEY, 'anthropic-version': '2023-06-01' }, body: JSON.stringify({ model, max_tokens: 1200, temperature: 0.2, messages: [{ role: 'user', content: buildAnalysisPrompt(payload) }] }) });
  const data = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error('Analysis temporarily unavailable.');
...
async function analyzeWithXAI(payload) {
  ...
  const response = await fetch('https://api.x.ai/v1/chat/completions', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${process.env.XAI_API_KEY}` }, body: JSON.stringify({ model, temperature: 0.2, response_format: { type: 'json_object' }, messages: [{ role: 'user', content: buildAnalysisPrompt(payload) }] }) });
  const data = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error('Analysis temporarily unavailable.');
...
async function analyzeWithGemini(payload) {
  ...
  const response = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ generationConfig: { temperature: 0.2, responseMimeType: 'application/json' }, contents: [{ role: 'user', parts: [{ text: buildAnalysisPrompt(payload) }] }] }) });
  const data = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error('Analysis temporarily unavailable.');
...
async function analyzeWithMicrosoft(payload) {
  ...
  const response = await fetch(endpoint, { method: 'POST', headers: { 'Content-Type': 'application/json', 'api-key': apiKey }, body: JSON.stringify({ model, temperature: 0.2, response_format: { type: 'json_object' }, messages: [{ role: 'user', content: buildAnalysisPrompt(payload) }] }) });
  const data = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error('Analysis temporarily unavailable.');
...
module.exports = { providers, providerStatuses, reviewWithProvider, buildAnalysisPrompt };

...
async function reviewWithAnthropic(analysis) {
  if (!process.env.ANTHROPIC_API_KEY) {
    return { ok: false, configured: false, message: 'Anthropic API is not configured. Add ANTHROPIC_API_KEY to enable Claude review.', notes: [] };
  }
  ...
  if (!response.ok) return { ok: false, configured: true, message: data.error?.message || 'Claude review request failed.', notes: [] };

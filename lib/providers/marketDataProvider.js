...
const MARKET_PROVIDER_ERROR = 'Market data provider request failed. Check backend configuration, provider plan, and limits.';
const MARKET_DATA_TIMEOUT_MESSAGE = 'Market data provider timed out. Analysis may be limited to Watch/No Trade.';
  ...
  try {
    ...
    if (error.name === 'AbortError') {
      return {
        ...
        symbol,
        interval,
        message: MARKET_DATA_TIMEOUT_MESSAGE,
...
function getMarketDataStatus() { return { configured: providerConfigured(), provider: providerName() || 'not-configured' }; }

module.exports = { getMarketData, getMarketDataStatus, PROVIDER_MESSAGE, MARKET_PROVIDER_ERROR, MARKET_DATA_TIMEOUT_MESSAGE, DEFAULT_SYMBOL_MAP, INTERVAL_MAP, resolveSymbol, resolveInterval };
